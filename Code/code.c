...
//********************************Read from eeprom*************************
//unsigned int read_students_from_eeprom(char student_list[][9]) {
//    unsigned int i = 0, j = 0, student_count = 0;
//
//    while (i < student_num * 8) { // Iterate through EEPROM space
//        char is_valid = 0;
//        for (j = 0; j < 8; j++) {
//            // Read each byte of the student ID
//            char byte = eeprom_read_byte(&(student[i + j]));
//
//            // Check if the byte is valid (not uninitialized)
//            if (byte != 0xFF) {
//                is_valid = 1; // Mark as valid
//            }
//            student_list[student_count][j] = byte == 0xFF ? '\0' : byte; // Store or null-terminate
//        }
//
//        if (is_valid) {
//            student_list[student_count][8] = '\0'; // Ensure null-termination
//            student_count++; // Increment valid student count
//        } else {
//            break; // Stop reading if no valid data
//        }
//
//        i += 8; // Move to the next student ID in EEPROM
//    }
//
//    return student_count; // Return the number of valid students
//}
//
//
//
...
//
//    char student_info[student_num][9];
//    unsigned int count;
//    unsigned int i=0;
//     USART_init(MYUBRR);
////    count=read_students_from_eeprom(student_info);
////    for( ;i<count;i++){
////        USART_TransmitString(student_info[i]);
////        USART_Transmit('\n');
////
////    }
//    USART_Transmit('a');
...
void display(char go){
  ...
  //********************Click on page 6********************
    ...
    else if(page==6 && go != 0xD1 && time==200){
...
//          for(;i<0x08;i++) code[i]=0x00;
    ...
    else if(page==4 && go != 0xD1 && time==200){
